...
//        информация о письмах (в порядке занесения в систему)
    ...
    private static void printOrderedByDateReceived(Set<Letter>letters) {
        System.out.println("\nВсе письма с сортировкой по дате получения ");
...
//        };
//
//        Set<Letter> letterForDate = new TreeSet<>(comparator);
//        letterForDate.addAll(letters);

//        for (Letter letter: letterForDate) {
//            System.out.println("Письмо от " + letter.authorName +  " поступило "  + letter.dateReceived);
//        }

        Set<Letter> sortedLetters = new TreeSet<>(Comparator.comparing(letter -> letter.dateReceived));
